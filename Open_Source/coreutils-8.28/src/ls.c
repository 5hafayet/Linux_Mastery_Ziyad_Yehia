...
enum
{
  TIME_STAMP_LEN_MAXIMUM = MAX(1000, INT_STRLEN_BOUND(time_t))
};
...
int main(int argc, char **argv)
{
  ...
  assert(ARRAY_CARDINALITY(color_indicator) + 1 == ARRAY_CARDINALITY(indicator_name));
  ...
  if (print_with_color)
  {
    ...
    if (is_colored(C_ORPHAN) || (is_colored(C_EXEC) && color_symlink_as_referent) || (is_colored(C_MISSING) && format == long_format))
      check_symlink_color = true;
  }

  if (dereference == DEREF_UNDEFINED)
    dereference = ((immediate_dirs || indicator_style == classify || format == long_format)
                       ? DEREF_NEVER
  ...
  format_needs_stat = sort_type == sort_time || sort_type == sort_size || format == long_format || print_scontext || print_block_size;
  format_needs_type = (!format_needs_stat && (recursive || print_with_color || indicator_style != none || directories_first));
  ...
  if (print_with_color && used_color)
  {
    ...
    if (!(color_indicator[C_LEFT].len == 2 && memcmp(color_indicator[C_LEFT].string, "\033[", 2) == 0 && color_indicator[C_RIGHT].len == 1 && color_indicator[C_RIGHT].string[0] == 'm'))
      restore_default_color();
...
decode_switches(int argc, char **argv)
{
...

  {
    ...
    if (p)
    {
      ...
      if (xstrtoul(p, NULL, 0, &tmp_ulong, NULL) == LONGINT_OK && tmp_ulong <= SIZE_MAX)
      {
        tabsize = tmp_ulong;
      }
      else
  ...
  while (true)
  {
    ...
    case COLOR_OPTION:
    {
      ...
      print_with_color = (i == when_always || (i == when_if_tty && isatty(STDOUT_FILENO)));
    ...
    case HYPERLINK_OPTION:
    {
      ...
      print_hyperlink = (i == when_always || (i == when_if_tty && isatty(STDOUT_FILENO)));
      break;
    }
  ...
  align_variable_outer_quotes = format != with_commas && format != one_per_line && (line_length || format == long_format) && (qs == shell_quoting_style || qs == shell_escape_quoting_style || qs == c_maybe_quoting_style);
  ...
  if ((time_type == time_ctime || time_type == time_atime) && !sort_type_specified && format != long_format)
  {
    sort_type = sort_time;
  }
...
{
  return ((ignore_mode != IGNORE_MINIMAL && name[0] == '.' && (ignore_mode == IGNORE_DEFAULT || !name[1 + (name[1] == '.')])) || (ignore_mode == IGNORE_DEFAULT && patterns_match(hide_patterns, name)) || patterns_match(ignore_patterns, name));
}
            ...
            bool command_line_arg, char const *dirname)
{
  ...
  if (command_line_arg || print_hyperlink || format_needs_stat
      ...
      || (type == directory && print_with_color && (is_colored(C_OTHER_WRITABLE) || is_colored(C_STICKY) || is_colored(C_STICKY_OTHER_WRITABLE)))
      /* When dereferencing symlinks, the inode and type must come from
         stat, but readdir provides the inode and type of lstat.  */
      || ((print_inode || format_needs_type) && (type == symbolic_link || type == unknown) && (dereference == DEREF_ALWAYS || color_symlink_as_referent || check_symlink_color))
      /* Command line dereferences are already taken care of by the above
         assertion that the inode number is not yet known.  */
      || (print_inode && inode == NOT_AN_INODE_NUMBER) || (format_needs_type && (type == unknown || command_line_arg
                                                                                 ...
                                                                                 || (type == normal && (indicator_style == classify
                                                                                                        ...
                                                                                                        || (print_with_color && (is_colored(C_EXEC) || is_colored(C_SETUID) || is_colored(C_SETGID) || is_colored(C_CAP))))))))

  {
    ...
    if ((type == normal || S_ISREG(f->stat.st_mode)) && print_with_color && is_colored(C_CAP))
    ...
    if (S_ISLNK(f->stat.st_mode) && (format == long_format || check_symlink_color))
    {
      ...
      if (linkname && (file_type <= indicator_style || check_symlink_color) && stat(linkname, &linkstats) == 0)
      {
        f->linkok = true;
        ...
        /* Symbolic links to directories that are mentioned on the
           ...
           listed as files.  */
        if (!command_line_arg || format == long_format || !S_ISDIR(linkstats.st_mode))
        {
...
{
  format_user_or_group(!stat_ok ? "?" : (numeric_ids ? NULL : getuser(u)), u, width);
}
...
{
  format_user_or_group(!stat_ok ? "?" : (numeric_ids ? NULL : getgroup(g)), g, width);
}
...
print_long_format(const struct fileinfo *f)
{
  ...
  *p = '\1';
  ...
  if (f->stat_ok && localtime_rz(localtz, &when_timespec.tv_sec, &when_local))
  {
    ...
    recent = (timespec_cmp(six_months_ago, when_timespec) < 0 && (timespec_cmp(when_timespec, current_time) < 0));
...
get_color_indicator(const struct fileinfo *f, bool symlink_target)
{
  ...
  if (type == C_FILE)
  {
    ...
    {
      if (ext->ext.len <= len && STREQ_LEN(name - ext->ext.len, ext->ext.string,
                                           ext->ext.len))
        break;
    }
  }
